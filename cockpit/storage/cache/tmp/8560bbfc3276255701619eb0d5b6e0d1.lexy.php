...
<div class="uk-grid" data-uk-grid-margin data-ng-controller="general-settings" ng-cloak>
    ...
    <div class="uk-width-medium-1-4">
        <ul class="uk-nav uk-nav-side" data-uk-switcher="{connect:'#settings-general'}">
            ...
            <li><a href="#GMAPS"><?php echo $app("i18n")->get('Google Maps'); ?></a></li>
        </ul>
    </div>
    ...
    <div class="uk-width-medium-3-4">
        <div class="app-panel">
            <div id="settings-general" class="uk-switcher">
                ...
                <div>
                    <span class="uk-badge app-badge"><?php echo $app("i18n")->get('Google Maps'); ?></span>
                    <hr />

                    <div class="uk-margin uk-form">
                        <label class="uk-text-small"><?php echo $app("i18n")->get('Google Maps API Key'); ?></label>
                        <input type="text" class="uk-form-large uk-width-1-1" ng-model="gmapskey" />
                    </div>

                    <button class="uk-button uk-button-large" type="button" ng-click="saveGmapsKey()"><?php echo $app("i18n")->get('Save'); ?></button>
                </div>
            </div>
        </div>
    </div>
...
<script>
    App.module.controller("general-settings", function($scope, $rootScope, $http, $timeout){
        ...
        $scope.gmapskey  = '<?php echo  $gmapskey ; ?>';
        ...
        $scope.saveGmapsKey = function() {
            $http.post(App.route("/settings/saveGmapsKey"), {"key": $scope.gmapskey}).success(function(data) {
                App.notify("<?php echo $app("i18n")->get('Key updated!'); ?>", "success");
            }).error(App.module.callbacks.error.http);
        };
